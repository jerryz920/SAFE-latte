...
object Identity {
  ...
  def computeSetToken(pid: String, label: String): String = {
    ...
    val selfCertifyingToken = if(label.isEmpty) s"${pid}" else s"${pid}:${labelHash}" 
    if(Config.config.selfCertifyingSetToken) {
      selfCertifyingToken
    } else {
      if(label.isEmpty) { return selfCertifyingToken }
      else {
        val hashToken = encode(hash(selfCertifyingToken.getBytes(StringEncoding), "SHA-256"), "base64URLSafe") 
        return hashToken
      }
    }
  }
