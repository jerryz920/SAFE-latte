...
defcon endorseUser(?User) :-
  spec(’GENI Member Authority endorses a user'),
  ?SubjectSet := label(“subject($Self)”),
  {
    geniUser($User).                (* $User is a GENI user *)
    link($SubjectSet).              (* pull in user's personal logic set *)
    label(“endorse/user/$User”).    (* label this logic set endorse/user/$User *)
  }.
```
