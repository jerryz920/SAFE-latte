...
class StrongBench(concurrency: Int, jvmmapFile: String, slangManager: SafelangManager) 
    extends SafeBench with StateMachineSetHelper with LazyLogging {

  val operators = Seq("groupMember", "nestGroup", "delegateMembership", "postGroupSet", 
                      ...
                      "delegateObjectThenQuery")

  val opcountMap = Map("groupMember"->2, "nestGroup"->2, "delegateMembership"->2, "postGroupSet"->1, 
  ...
  def run(): Unit = {
   ...
   for(i <- 1 to numObjects) { // Build the object hierarchy
     ...
     delegateObject(inference, rootObjectAuthority, objectRoot, objectName, objectId, rootObjectId)
  ...
  def delegateThenQueryNameAsync(inference: Safelang, delegator: PrincipalStub, user: PrincipalStub, queryPrincipal: PrincipalStub, queryJvm: String, objectName: String, toScid: String, homeScid: String, queryName: String): Unit = {
    val op = "delegateObjectThenQuery"
    val future = Future { 
      delegateObjectThenQuery(inference, delegator, user, queryPrincipal, queryJvm, objectName, toScid, homeScid, queryName)
    }
